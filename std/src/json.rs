...
pub fn native_json_stringify(args: &mut Vec<Value>) -> Result<Value, Value> {
    if args.is_empty() {
        return Err(Value::String("stringify requires at least one argument".to_string()));
    }
...
pub fn native_json_parse(args: &mut Vec<Value>) -> Result<Value, Value> {
    if args.is_empty() {
        return Err(Value::String("parse requires at least one argument".to_string()));
    }
    
    let json_str = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(Value::String("parse requires a string argument".to_string())),
    };
